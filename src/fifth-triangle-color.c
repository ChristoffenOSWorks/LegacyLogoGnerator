/*
 * The fifth triangle that has color.
 *
...
main (int argc, char *argv[])
{
	...
	/* Fifth triangle */
	cairo_set_source_rgb (cr, 0.60, 0.85, 0.91); //Sets the color of the triangle.
	cairo_line_to (cr, 244, 314); //Draws a line from Point A to Point B.
	cairo_line_to (cr, 148, 375); //Draws a line from Point B to Point C.
	cairo_line_to (cr, 70, 184); //Draws a line from Point C back to Point A.
	...
	cairo_set_source_rgb (cr, 0.60, 0.85, 0.91); //Sets the color of the triangle (again)
	cairo_stroke (cr); //Draw the final shape

	/* Write output and clean up */
        cairo_surface_write_to_png (surface, "../images/triangle5-color.png"); //Name the resulting image as triangle5-color.png
